...
@foreach ($options as $option)
    <form id="deleteForm_{{ $option->id }}" action="{{ route('admin.option.destroy', $option) }}" method="POST"
        ...
        @csrf
        @method('delete')
        <div class="modal fade" id="confirmModal_{{ $option->id }}" tabindex="-1" aria-labelledby="confirmModalLabel"
...
<script>
    function showConfirmModalOption(optionId) {
        let modalId = 'confirmModal_' + optionId;
